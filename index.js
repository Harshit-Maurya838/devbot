...
const commands = {
    ...
    profile: {
        ...
        execute: async (message) => {
            ...
            if (!username) {
                return message.reply("⚠️ Please provide a GitHub username. Usage: `!profile <GitHub Username>`");
            }

            try {
                const response = await axios.get(`https://api.github.com/users/${username}`);
                const { login, name, bio, public_repos, followers, following, avatar_url, html_url } = response.data;
                ...
                const profileEmbed = new EmbedBuilder()
                    ...
                    .addFields(
                        ...
                        { name: "📝 **Name**", value: name || 'Not Available', inline: true },
                        { name: "📝 **Bio**", value: bio || 'No bio available' },
                        { name: "📂 **Public Repositories**", value: public_repos.toString(), inline: true },
                ...
                console.error(error);
                message.reply("❌ Could not fetch the GitHub profile. Please check the username or try again later.");
            }
        }
    },
    repos: {
        ...
        execute: async (message, args) => {
            if (!args[0]) return message.reply("❌ Please provide a GitHub username, like `!repos Harshit-Maurya838`.");
            const username = args[0];
            try {
                const { data } = await axios.get(`https://api.github.com/users/${username}/repos?per_page=10&sort=updated`);
                const repoList = data.map(repo => `🔹 **[${repo.name}](${repo.html_url})**\n${repo.description || 'No description available'}\n⭐ ${repo.stargazers_count} | 🍴 ${repo.forks_count}\n`).join('\n');
                const reposEmbed = new EmbedBuilder()
                    ...
                    .setFooter({ text: `View all repositories: https://github.com/${username}?tab=repositories` })
                    .setTimestamp();
                message.channel.send({ embeds: [reposEmbed] });
            } catch (error) {
                console.error(error);
                message.reply("❌ Unable to fetch the repositories. Make sure the username is correct and try again.");
            }
        }
    },
    code: {
        ...
        execute: (message, args) => {
            ...
            if (!lang || !codeContent) {
                return message.reply("❌ Please provide a language and some code to share, like `!code js console.log('Hello, World!');`");
            }
            ...
            client.on('interactionCreate', async (interaction) => {
                if (!interaction.isButton() || interaction.customId !== 'copy_code') return;
                await interaction.reply({ content: `\`\`\`${lang}\n${codeContent}\n\`\`\``, ephemeral: true });
            });
        }
    },
    ping: {
        ...
        execute: (message) => {
            const helpEmbed = new EmbedBuilder()
                ...
                .addFields(
                    ...
                    { name: "🔹 **!code <language> <code>**", value: "Share code snippets. Example: `!code js console.log('Hello, World!');`" },
                    { name: "🔹 **!profile <GitHub Username>**", value: "Fetches GitHub profile information." },
                    { name: "🔹 **!repos <GitHub Username>**", value: "Displays recent repositories of a GitHub user." },
                    { name: "🔹 **!ping**", value: "Responds with 'Pong!' to check bot responsiveness." },
                    { name: "🔹 **!hello**", value: "Greets the user with a friendly message." },
...
client.on('messageCreate', (message) => {
    ...
    const command = commands[commandName.toLowerCase()]; // Remove the "!" and get the command name
    
    // Check if the command exists
    if (command) {
        if (command.isAdminOnly && !message.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
